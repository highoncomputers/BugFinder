...
class CSRFAgent(BaseAgent):
    ...
    async def execute(self, context: AgentContext) -> AgentResult:
        ...
        for form in forms:
            ...
            has_csrf = any(
                "csrf" in f.get("name", "").lower()
                or "token" in f.get("name", "").lower()
                or "_token" in f.get("name", "")
                for f in fields
            )
    ...
    def _extract_forms(self, html: str, base_url: str) -> list[dict]:
        ...
        for i, fm in enumerate(form_matches):
            ...
            for im in input_pattern.finditer(form_html):
                ...
                if name_m:
                    fields.append({"name": name_m.group(1), "type": re.search(r'type\s*=\s*["\']([^"\']*)["\']', input_tag, re.IGNORECASE)})
