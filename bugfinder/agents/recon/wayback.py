        ...
        try:
            ...
            if hasattr(resp, 'status_code') and resp.status_code == 200:
                ...
                if len(data) > 1:
                    ...
                    statuses = [row[2] if len(row) > 2 else "200" for row in data[1:]]

                    unique_urls = list(set(urls))[:200]
                    interesting = [u for u in unique_urls if any(
                        ext in u.lower() for ext in [".git", ".env", "admin", "api", "backup",
                                                      "config", "db", "sql", "dump", "password",
                                                      "secret", "token", "key", "aws", "s3", "debug",
                                                      "test", "staging", "dev", "internal", "swagger",
                                                      "graphql", "wp-", "phpinfo"]
                    )]
